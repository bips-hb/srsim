...
#' @param n_correlated_drugs Number of drug-pairs that will exhibit a nonzero correlation (Default: 5)
#' @param rho_drugs The correlation of the selected drug-pairs (Default: 0.25)
#' @param n_correlated_events Number of event-pairs that will exhibit a nonzero correlation (Default: 5)
#' @param rho_events The correlation of the selected event-pairs (Default: 0.25)
#' @param n_correlated_pairs Number of drug-event pairs that will exhibit a nonzero correlation (Default: 2)
#' @param rho The correlation of the selected drug-event pairs (Default: 0.25)
#' @param corrmat If \code{NULL} (default), generated by the given parameters 
...
#' 
#' @export
simulateSRData <- function(n_reports = 100, 
                           n_drugs = 10, 
                           n_events = 10,
                           alpha_drugs = 1.0,
                           beta_drugs = 5.0,
                           ...
                           n_correlated_drugs = 5, 
                           rho_drugs = 0.25,
                           n_correlated_events = 5,
                           rho_events = 0.25,
  ...
  if (is.null(corrmat)) {
    results <- generateCorrelationMatrix(
      prob_drugs,
      prob_events,
      n_correlated_drugs,
      rho_drugs,
    ...
    if (!results$valid) {
      stop(
        "Did not manage to find a valid correlation matrix in the given number of iterations (max_iter)"
      )
    }
    ...
    # check whether the given correlation matrix is indeed a correlation matrix
    
    # determine the covariance matrix 
    covmat <- corr2cov(corrmat, margprob)
    
    # We use the Cholesky decomposition to test for positive definiteness
    if (!corpcor::is.positive.definite(covmat, method = "chol")) {
      stop("Given correlation matrix is not positive definite")  
    }
  }
  
  samples <- generateReports(n_reports, qnorm(margprob), chol(covmat), valid_reports, n_drugs, n_events, verbose)
  
  # turn into something numeric
  samples <- as.numeric(samples)
  dim(samples) <- c(n_reports, n_drugs + n_events) 
  samples <- dplyr::as_tibble(samples)
  
  # add meaningful column names
  colnames(samples) <- c(sprintf("drug%d", 1:n_drugs), sprintf("event%d", 1:n_events))
